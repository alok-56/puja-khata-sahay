...
import { IndianRupee, Users, ShoppingCart, Wallet, Trash2, Search, Loader2, ChevronLeft, ChevronRight, Download } from "lucide-react";
...
export function Dashboard() {
  ...
  const loadInvoices = useCallback(async (page = 1, search = "", type?: "donation" | "expence") => {
    setIsLoading(true);
    try {
  ...
  useEffect(() => {
    loadSummary();
    loadInvoices(1, searchTerm, activeTab === "donations" ? "donation" : "expence");
  ...
  const handleAddDonation = (newDonation: any) => {
    ...
    loadSummary();
    loadInvoices(currentPage, searchTerm, activeTab === "donations" ? "donation" : "expence");
  };
  ...
  const handleDeleteDonation = async (donationId: string) => {
    ...
    try {
      ...
      toast({
        title: "Delete Failed",
        description: error instanceof Error ? error.message : "Failed to delete donation",
        variant: "destructive",
      });
    }
  };
  ...
  const handlePageChange = (page: number) => {
    if (page >= 1 && page <= totalPages) {
      setCurrentPage(page);
      loadInvoices(page, searchTerm, activeTab === "donations" ? "donation" : "expence");
    }
  };
  ...
  const donations = invoices.filter(invoice => invoice.invoice_type === "donation");
  const expenses = invoices.filter(invoice => invoice.invoice_type === "expence");
                ...
                <CardContent className="space-y-3 md:space-y-4">
                  {isLoading ? (
                    ...
                    donations.map((donation) => (
                      <div key={donation._id} className="border-l-4 border-primary pl-3 md:pl-4 py-3 bg-festival-cream/50 rounded-r-lg">
                        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2">
                          <div className="flex-1">
                            <h4 className="font-semibold text-base md:text-lg">{donation.name || 'Anonymous'}</h4>
                            <p className="text-sm text-muted-foreground">{donation.village}</p>
                            <p className="text-sm text-muted-foreground">{donation.number}</p>
                            ...
                            {isAdminLoggedIn && (
                              <div className="flex gap-1">
                                <DonationModal 
                                  ...
                                  onSave={(updatedDonation) => handleEditDonation(donation, updatedDonation)} 
                                  isEdit 
                                />
                                <Button 
                                  variant="ghost" 
                ...
                <CardContent className="space-y-3 md:space-y-4">
                  {isLoading ? (
                    ...
                    expenses.map((expense) => (
                      <div key={expense._id} className="border-l-4 border-destructive pl-3 md:pl-4 py-3 bg-destructive/5 rounded-r-lg">
                        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2">
                          <div className="flex-1">
                            <h4 className="font-semibold text-base md:text-lg">{expense.name || 'Unknown'}</h4>
                            <p className="text-sm font-medium text-destructive">{expense.expence_name || 'Expense'}</p>
                            ...
                            {isAdminLoggedIn && (
                              <div className="flex gap-1">
                                <ExpenseModal 
                                  ...
                                  onSave={(updatedExpense) => handleEditExpense(expense, updatedExpense)} 
                                  isEdit 
                                />
                                <Button 
                                  variant="ghost" 
